...
def get_series_data_code(series_code):        
    url = 'http://www.howstat.com/cricket/Statistics/Series/SeriesStats_ODI.asp?SeriesCode=0'+str(series_code)
    page = requests.get(url)
    soup = BeautifulSoup(page.content, 'html.parser')
    odi_series = soup.find_all('tr', attrs={'bgcolor': re.compile("#")})                                                             
    series_location = odi_series[0].find_all('td')
...
if __name__ == '__main__':
    ...
    download_dir = "records_final_series_odi_test2.csv" #where you want the file to be downloaded to 
